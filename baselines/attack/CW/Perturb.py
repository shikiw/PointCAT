...
class CWPerturb:
    ...
    def __init__(self, source_model, target_model=None, adv_func=None, dist_func=None, attack_lr=1e-2,
    ...
    def attack(self, data, target, mode='targeted'):
        ...
        for binary_step in range(self.binary_step):
            ...
            for iteration in range(self.num_iter):
                ...
                dist_loss = self.dist_func(adv_data, ori_data, torch.from_numpy(current_weight)).mean()
        ...
        with torch.no_grad():
            input = torch.from_numpy(o_bestattack).float().cuda()
            logits = self.target_model(input)  # [B, num_classes]
            if isinstance(logits, tuple):  # PointNet
                logits = logits[1]
