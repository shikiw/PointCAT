...
class AutoAttack():
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        if self.version != 'rand':
            check_randomized(self.model, x_orig[:bs].to(self.device),
                y_orig[:bs].to(self.device), bs=bs, logger=self.logger)
        check_range_output(self.model, x_orig[:bs].to(self.device),
            logger=self.logger)
        check_dynamic(self.model, x_orig[:bs].to(self.device), self.is_tf_model,
            logger=self.logger)
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # make sure that x is a 3d tensor even if there is only a single datapoint left
                    if len(x.shape) == 1:
                        x.unsqueeze_(dim=0)
