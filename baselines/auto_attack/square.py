...
class SquareAttack():
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas = torch.zeros([c, h]).to(self.device)
                    new_deltas[:, vh:vh + s] = 2. * self.eps * self.random_choice([c, 1])
                ...
                for _ in range(h // s):
                    for _ in range(1):
                        delta_init[:, :, vh:vh + s] += self.eta(
                            s).view(1, 1, s) * self.random_choice(
                            [x.shape[0], c, 1])
                    vh += s
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas_mask[:, :, vh:vh + s] = 1.0
                    norms_window_1 = (delta_curr[:, :, vh:vh + s] ** 2).sum(dim=-1, keepdim=True).sqrt()

                    vh2 = self.random_int(0, h - s)
                    new_deltas_mask_2 = torch.zeros_like(x_curr)
                    new_deltas_mask_2[:, :, vh2:vh2 + s] = 1.

                    norms_image = self.lp_norm(x_best_curr - x_curr)
                    mask_image = torch.max(new_deltas_mask, new_deltas_mask_2)
                    norms_windows = ((delta_curr * mask_image) ** 2).sum(dim=-1, keepdim=True).sqrt()

                    new_deltas = torch.ones([x_curr.shape[0], c, s]
                        ).to(self.device)
                    new_deltas *= (self.eta(s).view(1, 1, s) *
                        self.random_choice([x_curr.shape[0], c, 1]))
                    old_deltas = delta_curr[:, :, vh:vh + s] / (
                        1e-12 + norms_window_1)
                    new_deltas += old_deltas
                    new_deltas = new_deltas / (1e-12 + (new_deltas ** 2).sum(
                        dim=-1, keepdim=True).sqrt()) * (torch.max(
                        (self.eps * torch.ones_like(new_deltas)) ** 2 -
                        norms_image ** 2, torch.zeros_like(new_deltas)) /
                        c + norms_windows ** 2).sqrt()
                    delta_curr[:, :, vh2:vh2 + s] = 0.
                    delta_curr[:, :, vh:vh + s] = new_deltas + 0
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas_mask = torch.zeros_like(x_curr)
                    new_deltas_mask[:, :, vh:vh + s] = 1.0
                    norms_window_1 = delta_curr[:, :, vh:vh + s].abs().sum(dim=-1, keepdim=True)
                    ...
                    norms_image = self.lp_norm(x_best_curr - x_curr)
                    mask_image = torch.max(new_deltas_mask, new_deltas_mask_2)
                    norms_windows = (delta_curr * mask_image).abs().sum(dim=-1, keepdim=True)
    ...
    def perturb(self, x, y=None):
        ...
        if not self.targeted:
            acc = self.predict(x)[1].max(1)[1] == y
        else:
            acc = self.predict(x)[1].max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict(adv_curr)[1]
                if not self.targeted:
